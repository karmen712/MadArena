...
class Human:
    def __init__(self, pos, max_hp, state, team):
        ...
        self.animations = HumanAnimations(200)
        # region ATTACK
    ...
    def close_to_attack_target(self):
        if self.end_fight():
            self.stop_attacking()
            return
        ...
        distance_to_move = self.attack_range / 2.5
    ...
    def draw(self, screen):
        ...
        def walk_to_target():
            ...
            tx, ty = self.target
            x_condition = bx - (self.half_rect.width*0.6) <= tx <= bx + (self.half_rect.width*0.6)
            y_condition = by - (self.body_height*0.6) <= ty <= by + (self.body_height*0.6)
...
# if self.attack_stand_time > 0:
#     self.attack_stand_time - options.milliseconds
#     if self.dir == 0:
#         self.image = image.load(resource_path("Media/Sprites/Units/Human/Attack/Left/human_attack_left3.png"))
#     else:
#         self.image = image.load(resource_path("Media/Sprites/Units/Human/Attack/Right/human_attack_right3.png"))
#     screen.blit(self.image, self.rect)
#     return
